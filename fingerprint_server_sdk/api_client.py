...
import fingerprint_server_sdk.models
from fingerprint_server_sdk import __version__, rest
from fingerprint_server_sdk.api_response import ApiResponse
from fingerprint_server_sdk.api_response import T as ApiResponseT
...
FilesType = dict[
    str, Union[str, bytes, list[str], list[bytes], tuple[str, bytes], list[tuple[str, bytes]]]
]
...
class ApiClient:
    ...
    def param_serialize(
        ...
        # post parameters
        post_params_result: Any = None
        if post_params or files:
            post_params_list = post_params if post_params else []
            post_params_sanitized = self.sanitize_for_serialization(post_params_list)
            post_params_result = self.parameters_to_tuples(
                post_params_sanitized, collection_formats
            )
            if files:
                post_params_result.extend(self.files_parameters(files))
        ...
        if getattr(self.configuration, "default_query_params", None):
            ...
            for k, v in self.configuration.default_query_params:
        ...
        return method, url, header_params, body, post_params_result
    ...
    def call_api(
        ...
        header_params: Optional[dict[str, Any]] = None,
        body: Any = None,
        post_params: Optional[list[Any]] = None,
    ...
    def sanitize_for_serialization(self, obj: Any) -> Any:
        ...
        if isinstance(obj_dict, list):
            # here we handle instances that can either be a list or something else,
            # and only became a real list by calling to_dict()
    ...
    def __deserialize(self, data: Any, klass: Any) -> Any:
        ...
        if isinstance(klass, str):
            ...
            if klass in self.NATIVE_TYPES_MAPPING:
                ...
                klass = getattr(fingerprint_server_sdk.models, klass)
    ...
    def update_params_for_auth(
        ...
        queries: Optional[list[tuple[str, Any]]],
            ...
            for auth in auth_settings:
                ...
                if auth_setting:
                    self._apply_auth_params(
                        ...
                        method,
                        body,
                        auth_setting,  # type: ignore[arg-type]
                    )
    ...
    def __deserialize_file(self, response: rest.RESTResponse) -> Any:
        ...
        :return: file path.
        """
        fd, path = tempfile.mkstemp(dir=getattr(self.configuration, "temp_folder_path", None))
        os.close(fd)
        os.remove(path)
